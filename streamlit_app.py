...
# 1. Konfigūracija
st.set_page_config(page_title="ETH V51 QUANTUM MASTER", layout="wide")
...
if kainos and len(kainos) > 50:
    ...
    # Efficiency Ratio (Harvard metodas rinkos triukšmui filtruoti)
    ...
    tikimybe = (er * 100) + (10 if momentum > 0 else -5) # Modifikuota tikimybė

    # --- CRYPTO CRITICAL BLOKAS (Virsutine juosta) ---
    st.markdown(f"""
    <div style="background-color:{'#28a745' if momentum > 0 else '#dc3545'}; padding:15px; border-radius:10px; text-align:center; color:white;">
    ...
    # --- PROGNOZĖ (Kalibruotas Tikimybinis Modelis) ---
    l_fut = [laikai[-1] + timedelta(minutes=30*h) for h in range(1, 11)]
    p_fut = []
    for h in range(1, 11):
        # Bangų amplitudė priklauso nuo rinkos efektyvumo (er)
        amplituda = volatilumas * (1.2 - er) * (1 if h % 2 == 0 else -1)
        trendas = momentum * h * er
        p_fut.append(dabartine + trendas + amplituda)

    fig, ax = plt.subplots(figsize=(12, 8), facecolor='black')
    ax.set_facecolor('#0a0a0a')
    
    # 1. 4 VALANDŲ ISTORIJA (Išlaikome platų vaizdą)
