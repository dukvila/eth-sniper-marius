...
# 1. Konfigūracija
st.set_page_config(page_title="ETH V50 PROBABILITY SNIPER", layout="wide")
...
if kainos and len(kainos) > 50:
    ...
    # --- ANALIZĖS MECHANIZMAS (PROBABILITY ENGINE) ---
    momentum = (kainos[-1] - kainos[-12]) / 3.0
    volatilumas = statistics.stdev(kainos[-40:])
    
    # Efficiency Ratio (Harvard metodas)
    ...
    tikimybe = er * 100 # Konvertuojame į procentus

    # --- CRYPTO CRITICAL BLOKAS ---
    st.markdown(f"""
    <div style="background-color:{'#28a745' if momentum > 0 else '#dc3545'}; padding:20px; border-radius:10px; text-align:center; color:white;">
        <h2 style="margin:0;">🚀 {'VERTA PIRKTI' if momentum > 0 else 'LAUKITE KOREKCIJOS'}</h2>
        <p style="margin:5px; font-size:18px;">Pikas: {(dabartine + volatilumas):.1f}€ | Tikimybė: {tikimybe:.1f}%</p>
    </div>
    ...
    p_fut = []
    for h in range(1, 11):
        # Logiškas zigzaginis judėjimas pagal rinkos jėgą
        triuksmas = (volatilumas * 0.7 * (1 if h % 2 == 0 else -1))
        trendas = momentum * h * er
    ...
    ax.plot(laikai[idx:], kainos[idx:], color='#1a46ba', linewidth=4, label="Praeitis")
    
    # 2. PROGNOZĖS LINIJA
    ax.plot(l_fut, p_fut, color='#00ffcc', linewidth=5, marker='o', markersize=3)
    
    # 3. SUMOS ANT VISŲ LŪŽIŲ (Privaloma telefono ekranui)
    for i in range(len(p_fut)):
        if i > 0 and i < len(p_fut)-1:
            ...
            if is_peak or is_bottom:
                color = 'white' if is_peak else '#ff4b4b'
                ax.text(l_fut[i], p_fut[i] + (1.2 if is_peak else -2.5), 
                        f"{p_fut[i]:.1f}€", color=color, fontweight='bold', ha='center', fontsize=10)

    # 4. ŽALIAS TIKSLO APSKRITIMAS (Ant linijos galo)
