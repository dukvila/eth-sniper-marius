...
# 1. Konfigūracija
st.set_page_config(page_title="ETH V59 STABLE SNIPER", layout="wide")
...
if kainos and len(kainos) > 50:
    ...
    # --- HARVARD ANALIZĖS VARIKLIS ---
    # Momentum (kainos greitis per 3h)
    momentum = (kainos[-1] - kainos[-12]) / 3.0
    volatilumas = statistics.stdev(kainos[-40:])
    
    # Rinkos efektyvumas (ER) - Harvard triukšmo filtras
    ...
    # --- GRAFIKAS TELEFONUI (Visiškai pataisytas) ---
    ...
    # Zig-Zag prognozės linija
    ax.plot(l_fut, p_fut, color='#00ffcc', linewidth=5, marker='o', markersize=4)
    
    # SUMOS ANT VISŲ LŪŽIŲ (Ištaisyta NameError ir SyntaxError)
